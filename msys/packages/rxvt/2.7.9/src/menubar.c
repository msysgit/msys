 ...
 * File:	menubar.c
 *----------------------------------------------------------------------*
 * $Id: menubar.c,v 1.1 2003/03/05 17:33:37 earnie Exp $
 *
...
rxvt_action_dispatch(rxvt_t *r, action_t *action)
{
    switch (action->type) {
    case MenuTerminalAction:
	rxvt_cmd_write(r, action->str, action->len);
	break;

    case MenuAction:
	rxvt_tt_write(r, action->str, action->len);
	break;
...
/* INTPROTO */
void
rxvt_drawtriangle(rxvt_t *r, int x, int y, int state)
{
    GC              top, bot;
    int             w;
...
/* determine the correct right-alignment */
	...
	if (isSeparator(item->name)) {
		...
		rxvt_drawtriangle(r, ActiveMenu->w, y, +1);
...
/* EXTPROTO */
void
rxvt_menubar_read(rxvt_t *r, const char *filename)
{
...
/* EXTPROTO */
void
rxvt_menubar_dispatch(rxvt_t *r, char *str)
{
    int             n, cmd;
	...
	while (str[0] == '[') {
		...
		*next = '\0';
		...
		rxvt_map_menuBar(r, 1);
		r->h->menu_readonly = 1;
	    } else if (!STRCMP(str, "hide")) {
		rxvt_map_menuBar(r, 0);
		r->h->menu_readonly = 1;
	    } else if ((n = rxvt_Str_match(str, "read:")) != 0) {
		/* read in a menu from a file */
		str += n;
		rxvt_menubar_read(r, str);
		...
		str += n;
		rxvt_xterm_seq(r, XTerm_Pixmap, str, CHAR_ST);
	    }
#if (MENUBAR_MAX > 1)
		...
		if ((fp = fopen(buffer, "wb")) != NULL) {
		    rxvt_xterm_seq(r, XTerm_title, buffer, CHAR_ST);
...
/* INTPROTO */
int
rxvt_menubar_mapping(rxvt_t *r, int map)
{
    int             change = 0;
	...
	if (menu == NULL && r->h->Arrows_x && ev->x >= r->h->Arrows_x) {
		...
		if (ev->x >= (r->h->Arrows_x + (Width2Pixel(4 * i + i)) / 4)
			...
			if (Arrows[i].str != NULL && Arrows[i].str[0] != 0)
			    rxvt_tt_write(r, (Arrows[i].str + 1),
					  Arrows[i].str[0]);
		    }
