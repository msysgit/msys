 ...
 */
		...
		if (h->rs[Rs_answerbackstring])
		    rxvt_tt_write(r,
			(const unsigned char *)h->rs[Rs_answerbackstring],
			(unsigned int)STRLEN(h->rs[Rs_answerbackstring]));
		else
